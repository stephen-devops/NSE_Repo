...
async function getInitialData() {
	...
	const query2 = `MATCH (i:IP)-[r]-(n:Node), (n)-[r2]-(h:Host), ` +
			...
			`WHERE i.address STARTS WITH "${ipPrefix}" ` +
			`RETURN i, count(v);`;
	...
	vulnResult.records.forEach(vulnRecord => {
        	...
        	if (vulnValue.labels) {
                	...
                	// Ensure the IP address has an entry in the dictionary
                	if (!vulnNodeHosts.hasOwnProperty(ipNodeAddress)) {
                    	    vulnNodeHosts[ipNodeAddress] = {
                        	label: ipNodeAddress,
            	    ...
            	    // vulnValue is the count(v)
            	    countValue = vulnValue.low;

            	    if (ipNodeAddress) {

                    	if (vulnNodeHosts[ipNodeAddress]) {
	            	    vulnNodeHosts[ipNodeAddress].value = countValue;
                    	} else {
                    	    console.error(`IP Address ${ipNodeAddress} not found in vulnNodeHosts.`);
                    	}
            	    } else {
	...
	// console.log('No. of IP Hosts: ', nodeHosts);
	// console.log('No. of Vulnerable Node Hosts: ', Object.keys(vulnNodeHosts).length);
...
async function buildCIDRTreemap(supernet, cidrDict) {
    ...
    function insertIntoTree(node, cidr, label) {
        ...
        if (mostSpecificParent) {
            insertIntoTree(mostSpecificParent, cidr, label);
        } else {
            node.children.push({ cidr, label, children: [] });
        }
    }
    ...
    for (const [cidr, { value, label }] of sortedCIDRDictEntries) {
        if (isInSubnet(value, supernet)) {
            insertIntoTree(treemap[supernet], value, label);
        }
    }
...
async function populateVirtualNetwork(data) {
    ...
    if (Array.isArray(data)) {
        ...
        data.forEach(element => {
            const { id, source, target, label, type, parent, details, hosts } = element.data;
		...
		const nodeData = { label, type, details, hosts };
