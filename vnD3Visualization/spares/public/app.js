...
async function initializeD3Visualization(supernet, data) {
    ...
    let treemapId = null;
    let treemapLabel = null;
    let treemapDetails = null;

    const cidrUnit = data.find(el => el.data && el.data.type === 'CIDR_Values');

    if (cidrUnit) {
	treemapId = cidrUnit.data.id;
	treemapLabel = cidrUnit.data.label;
	treemapDetails = cidrUnit.data.details;
    }
	...
	group: 'nodes',
	    ...
	    details: treemapDetails,
            treemapSVG: treemapSVG // Include the SVG as a property
        },
        // position: { x: 400, y: 200 },
    };

    data.push(treeMapNode);

    // Render nodes
    const nodes = data.filter(d => !d.data.source && !d.data.target);

    nodes.forEach(node => {
	const { id, type, treemapSVG } = node.data;

	if (type === "CIDR_Treemap" && treemapSVG) {
	...
	dataHeight = root.height;
    	...
    	// Custom tiling function to assign coordinates
    	const customTiling = (node, x0, y0) => {

	    // Modify propertys based on the conditions
	    if (node.data && node.data.cidr) {

		const cidr = node.data.cidr;

		if (cidr.endsWith("/29") || cidr.endsWith("/30") || cidr.endsWith("/31")) {
		    ...
		    if (vuln !== undefined){
			    ...
			    node.data.color = "crimson"; // Add a dynamic property for the error
		    	} else if (vuln === 0){

            	            node.data.color = "aqua"; // Add a custom property for the rectangle color
		    	}
		    }
        	}
	    }
    	...
    	// Apply custom tiling starting from the root node
